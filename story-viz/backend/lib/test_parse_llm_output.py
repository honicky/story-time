...
def test_find_first_json_array_of_strings():
    ...
    assert (
        find_first_json_array_of_strings('Prefix text ["First", "Second"] suffix text ["Third", "Fourth"] suffix2')
        == '["First", "Second"]'
    )
...
def test_count_chars_outside_first_json_array():
    ...
    assert (
        count_chars_outside_first_json_array('Prefix text ["First", "Second"] suffix text') == 24
    )  # Length of 'Prefix text ' and ' suffix text'
    assert (
        count_chars_outside_first_json_array('Prefix text ["First", "Second"] suffix text ["Third", "Fourth"] suffix2')
        == 52
    )  # Length of 'Prefix text ' and ' suffix text ["Third", "Fourth"] suffix2'
    assert (
        count_chars_outside_first_json_array('No JSON here!') == 13
    )  # Entire input is considered outside since there's no JSON array
