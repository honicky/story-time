...
from . import object_store_client
...
class NextLegClient:
    ...
    def _fetch_to_completion(self, message_id, ppu_id, retry_count=0):
        image_res = requests.get(f"{self.ppu_url}/message/{message_id}?ppuId={ppu_id}", headers=self.headers)
        ...
        if image_response_data.get("progress") and image_response_data.get("progressImageUrl"):
            print("---------------------")
    ...
    def generate_image(self, prompt):
        image_res = requests.post(
            ...
            json={"msg": f"{prompt} --relax --v 6"},
            timeout=120,
        )
        image_response_data = image_res.json()

        self._check_for_errors(image_response_data)  # Check for error codes using the custom exception
        ...
        completed_image_data = self._fetch_to_completion(image_response_data["messageId"], image_response_data["ppuId"])
...
class NextLegException(Exception):
    ...
    def __init__(self, error_code):
        ...
        self.error_description = self.error_descriptions.get(error_code, "Unknown error code")
...
class ReplicateClient:
    ...
    def generate_image(self, prompt, negative_prompt=""):
        ...
        if self.models[self.model_name]["supports_multiple"]:
            image_urls = replicate.run(model["id"], input=model["parameters"])
        else:
            image_urls = [replicate.run(model["id"], input=model["parameters"])[0] for _ in range(self.image_count)]
...
def copy_images_to_s3(image_urls):
    ...
    return [
        construct_image_url(
            client.upload_from_url(image_url, "botos-generated-images", f"generated-images/{generation_id}", id)
        )
